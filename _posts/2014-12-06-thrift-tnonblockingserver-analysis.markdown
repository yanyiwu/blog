...
最幸运的是，Thrift 的 TNonblockingServer 实现代码干净，注释丰富，
...
第二种比较有意思，这个事件对应的文件描述符是 socket pair ，使用 evutil_socketpair 
...
而在 TNonblockingServer 里面，则通过该管段进行消息的传递。
...
按 frame 来一帧帧的接受。所以第一个状态值 `SOCKET_RECV_FRAMING` 
...
则运行该函数的过程整个主线程就会被阻塞(传说中的的 block the whole world)，
...
鄙人读演员的时候习惯打 `cout` ，然后跑起来看结果，
下面有一份运行示例可以帮助理解。

## 运行示例

对应的被我修改过的源码是 [MyTNonblockingServer] ：
